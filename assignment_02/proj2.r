...
plot_real_vs_simulated <- function(days, real_deaths, simulated_deaths, lower_bounds, upper_bounds, iteration) {
  ...
  plot <- ggplot(plot_data, aes(x = Day)) +
    ...
    geom_line(aes(y = SimulatedDeaths, color = "Simulated Deaths"), linewidth = 1) +
    geom_ribbon(aes(ymin = LowerBound, ymax = UpperBound), fill = "blue", alpha = 0.2) +
    labs(title = paste("Iteration:", iteration), x = "Day", y = "Deaths") +
    scale_color_manual(values = c("Real Deaths" = "red", "Simulated Deaths" = "blue"))
  
  plot <- plot + geom_vline(xintercept = 84, linetype = "dashed", color = "black") +
    annotate("text", x = 84, y = max(real_deaths, na.rm = TRUE), label = "UK Lockdown\nMarch 24, 2020", hjust = -0.1, color = "black")
  
  ggsave(file.path("plots", paste("iteration_plot_", iteration, ".png", sep = "")), plot = plot, width = 10, height = 6, bg = "white")
}

plot_p_statistic <- function(p_values) {
  p_statistic_data <- data.frame(Iteration = seq_along(p_values), PStatistic = p_values)
  
  p_statistic_plot <- ggplot(p_statistic_data, aes(x = Iteration, y = PStatistic)) +
    geom_line(color = "blue") +
    geom_point(color = "blue") +
    labs(x = "Iteration", y = "P Statistic", title = "P Statistic Over Iterations")
  
  ggsave(file.path("plots", "p_statistic_plot.png"), plot = p_statistic_plot, width = 10, height = 6, bg = "white")
}
...
deconvolution <- function(days, real_deaths, num_reps = 100, bootstrap = FALSE, initial_t0 = NULL) {
  ...
  for (rep in seq_len(num_reps)) {
    ...
    if (rep == 1 || rep %% 5 == 0) {
      plot_real_vs_simulated(days, real_deaths, daily_deaths, lower_bounds[, rep], upper_bounds[, rep], rep)
    }
